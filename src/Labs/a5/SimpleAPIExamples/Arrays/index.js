...
function WorkingWithArrays() {
    const API = "https://kanbas-node-server-app-chen-f1c054a4dcda.herokuapp.com/a5/todos";
    ...
    return (
        <div>
            ...
            <textarea onChange={(e) => setTodo({ ...todo, description: e.target.value })} value={todo.description} type="text" className="form-control" />
            <input onChange={(e) => setTodo({ ...todo, due: e.target.value })} value={todo.due} type="date" className="form-control" />
            <label>
                <input onChange={(e) => setTodo({ ...todo, completed: e.target.checked })} value={todo.completed} type="checkbox" className="form-check-input" />
                Completed
            </label>
            ...
            <ul className="list-group">
                {
                    todos.map((item) => (
                        <li key={item.id} className="list-group-item">
                            <button onClick={() => fetchTodoById(item.id)} className="btn btn-warning me-2 float-end">Edit</button>
                            <button onClick={() => removeTodos(item)} className="btn btn-danger float-end">Remove</button>
                            <input checked={item.completed} type="checkbox" readOnly className="form-check-input" />
                            {item.title}
            ...
            <div className="form-check">
                <input type="checkbox" onChange={(e) => setTodo({ ...todo, completed: e.target.checked })} className="form-check-input" id="completed"
                    name="completed" />
                <label class="form-check-label" for="completed">
                    Checked For True, Uncheck for False
                </label>
